...
def qsgd_compk(eta_grad, memory, topK_flag, frac, s):
    ...
    def qsgd(var):
        ...
        beta = tf.cast(K, dtype=tf.float32)/(s**2.0)
        return unnormalized/(1.0+beta)
...
#            memory = opt.get_slot(var, "memory")
            tensor_quantized, error = qsgd_compk(tensor, memory, topK_flag=1, frac=0.001, s=2)
